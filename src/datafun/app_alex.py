...
def bar_plot_example(
    ...
    sns.set(font_scale=0.7)  # pyright: ignore[reportDeprecated]
...
def narrow_down_top_occupations(df_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    top_area: str = df_clean.groupby(GROUP_COL)[EXAMPLE_NUMERIC_COL].mean().idxmax()  # pyright: ignore[reportAssignmentType]
